...
namespace BrickGame
{
    ...
    public class Ball
    {
        ...
        public int Collision(int x, int y)
        {
            if (y == 0) //벽충돌
            {
                m_tBall.nDirect = g_WallCollision[0, m_tBall.nDirect]; //0,1 -> 2방향
            ...
            //Bar충돌처리
            if (x >= m_Bar.m_tBar.nX[0] && x <= m_Bar.m_tBar.nX[2] + 1 &&
                y == (m_Bar.m_tBar.nY)) //바 위 충돌
            {
        ...
        {
            m_tBall.nReady = 0; //공 움직이면 0 안움직이면 1
            m_tBall.nDirect = 1;
            m_tBall.nX = 30;
            m_tBall.nY = 10;
